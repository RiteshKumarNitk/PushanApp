-- OPTIMIZED SCHEMA FOR TEA APP (BULK ORDERS) - IDEMPOTENT VERSION
-- This schema consolidates necessary tables and handles existing objects gracefully.
...
-- Products: Readable by everyone, Writable by Admin (Authenticated)
...
-- NOTE: For stricter security, replace 'authenticated' with check for admin role in users table
...
-- We also need to explicitly Drop conflicting "Auth manage variants" if it exists from previous migrations
DROP POLICY IF EXISTS "Auth manage variants" ON public.product_variants;

-- Orders: Users see their own, Admins see all
DROP POLICY IF EXISTS "User view own orders" ON public.tea_orders;
CREATE POLICY "User view own orders" ON public.tea_orders FOR SELECT USING (auth.uid() = user_id OR auth.role() = 'service_role');

DROP POLICY IF EXISTS "Admin manage orders" ON public.tea_orders;
CREATE POLICY "Admin manage orders" ON public.tea_orders FOR ALL USING (auth.role() = 'authenticated');

-- Order Items: Users see items of their orders
DROP POLICY IF EXISTS "User view own items" ON public.tea_order_items;
...
DROP POLICY IF EXISTS "Admin manage items" ON public.tea_order_items;
...
DROP POLICY IF EXISTS "User view history" ON public.supercoin_history;
CREATE POLICY "User view history" ON public.supercoin_history FOR SELECT USING (auth.uid() = user_id);

-- Clean up older conflicting policies if any
